...
auth = Auth(env)
blog = Blog(env)
blog_helper = blog_helper.BlogHelper(env, auth, blog)
...
def test_create_post(test_context, test_case, username, password, title, content, slug):
    ...
    result = True
    response, login = blog_helper.create_post(username=username, password=password, title=title, content=content, slug=slug)
    ...
    if response.status_code == 200:
        ...
        logger.info("Unable to create post " + response.status_code)
        result &= False
    assert result
